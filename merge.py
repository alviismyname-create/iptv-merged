...
MOVED_TAMIL_SET = {name.lower() for name in MOVED_FROM_TAMIL}
...
# ------------------------------------------------------------
# STEP 2: Download Tamil, BUT capture moved channels for Malayalam
# ------------------------------------------------------------
tam_channels = download_playlist("https://iptv-org.github.io/iptv/languages/tam.m3u")
tamil_kept = []
tam_removed = 0
moved_channels = []  # These will go to Malayalam group
...
for extinf, url in tam_channels:
    ...
    # Check if this is one of the channels to move to Malayalam
    if clean_name(full_name) in MOVED_TAMIL_SET:
        # Add to Malayalam group instead
        extinf_new = set_group_and_logo(extinf, GROUP_NAME)
        moved_channels.append((extinf_new, url))
        print(f"   ➕ Moved to {GROUP_NAME}: {full_name}")
    elif clean_name(full_name) in TAMIL_SET:
        # Keep in Tamil group
...
# Add moved channels to Malayalam group
